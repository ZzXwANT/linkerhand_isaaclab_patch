...
# Pre-defined configs
##
from isaaclab_assets.robots.linker import L20LITE_CFG  # isort: skip

# Explicitly actuated joints for the L20 Lite asset.
L20LITE_ACTIVE_JOINTS = [
    "thumb_cmc_roll",
    "thumb_cmc_yaw",
    "thumb_cmc_pitch",
    "index_mcp_roll",
...
@configclass
class L20LiteKinematicObsGroupCfg(ObsGroup):
    """Observations for the L20 Lite in-hand task."""
...
@configclass
class L20LiteCubeEnvCfg(inhand_env_cfg.InHandObjectEnvCfg):
    def __post_init__(self):
        # Override observations and actions BEFORE calling parent __post_init__
        self.observations.policy = L20LiteKinematicObsGroupCfg()
        self.actions = L20LiteActionsCfg()
        
        # post init of parent
        super().__post_init__()

        # switch robot to L20 Lite asset
        self.scene.robot = L20LITE_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        # Keep the current experiment setup unchanged while this task is revalidated.
...
@configclass
class L20LiteCubeEnvCfg_PLAY(L20LiteCubeEnvCfg):
    def __post_init__(self):
        # post init of parent
        super().__post_init__()
